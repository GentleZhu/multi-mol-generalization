...
path = "/home/shivama2/pre-training-via-denoising/data/qm9/raw/gdb9.sdf"
...
def train_val_test_split2(dataset, train_size, val_size, test_size, mols):
    dset_len = len(dataset)
    assert (train_size is None) + (val_size is None) + (
        test_size is None
    ) <= 1, "Only one of train_size, val_size, test_size is allowed to be None."
...
def make_splits(
    ...
    if splits is not None:
        ...
        # )
        idx_train, idx_val, idx_test = train_val_test_split2(
            dataset, train_size, val_size, test_size, data_mols
        )
        # print(np.shape(idx_train),np.shape(idx_test), np.shape(idx_val))
        # merged = np.sort(np.concatenate([idx_train, idx_val, idx_test], axis=0))
        
        # tester = np.array(range(110000+10831+10000))
        # print(merged)
        # print(tester)
        # for i in merged:
        #     if i not in tester:
        ...
        #     # else:
        #         # print('j')
        # print(np.array_equal(merged,tester))
        # # print(merged.shape,tester.shape)
        # # print(merged==tester.all())
        # # print
        # exit(0)
